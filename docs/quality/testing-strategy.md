...
Component/Unit and Component Integration levels need no live external services — fast, free, deterministic; they're what the CI/CD fast gate runs on every push ([Operations: CI/CD](../operations/ci-cd.md)).

Directory layout: [Design: Subsystem Design](../design/subsystem-design.md#project-layout). Task entry points: `task test:unit`, `task test:integration`, `task test:system`, `task test:acceptance`, `task test:all` ([Operations: Task Automation](../operations/task-automation.md)).
